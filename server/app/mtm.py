...
초안 데모 시나리오는 "달러 강세 → 체결 선물환 평가손"이었다. 그런데 데모의 체결 건
(나래상사)은 **수입**이고 매수 선물환이다 — 달러 강세(원화 약세)에서는 계약환율이 시장보다
...
def adverse_direction(pos: str) -> dict:
    ...
    이걸 명시하는 이유: 화면이 "달러 강세 = 위험"처럼 방향을 고정해 말하면 수입기업에게
    거짓말이 된다. 수입기업의 헤지는 달러 강세에서 이익이 나고, 원화 강세에서 아프다.
    """
    if pos == "export":
        return {"direction": "up",
                "label": "원화 약세(환율 상승)",
